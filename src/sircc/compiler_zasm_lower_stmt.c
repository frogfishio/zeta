...
bool zasm_emit_call_stmt(
  ...
  if (!args || args->type != JSON_ARRAY || args->v.arr.len < 1) {
    zasm_err_node_codef(p, call_id, n->tag, "sircc.zasm.call.bad_args", "sircc: zasm: %s node %lld missing args array", n->tag,
                        (long long)call_id);
    return false;
  }
  ...
  if (!parse_node_ref_id(p, args->v.arr.items[0], &callee_id)) {
    zasm_err_node_codef(p, call_id, n->tag, "sircc.zasm.call.bad_args", "sircc: zasm: %s node %lld args[0] must be node ref", n->tag,
                        (long long)call_id);
    return false;
  }
  ZasmOp callee = {0};
  if (!zasm_lower_value_to_op(p, strs, strs_len, allocas, allocas_len, names, names_len, bps, bps_len, callee_id, &callee) || callee.k != ZOP_SYM) {
    zasm_err_node_codef(p, call_id, n->tag, "sircc.zasm.call.bad_callee",
                        "sircc: zasm: %s node %lld callee must be a direct symbol (decl.fn/ptr.sym)", n->tag, (long long)call_id);
    return false;
  }
  ...
  if (!lowered) {
    zasm_err_node_codef(p, call_id, n->tag, "sircc.zasm.oom", "sircc: zasm: out of memory");
    return false;
  }
  lowered[0] = callee;
  ...
  for (size_t i = 1; i < args->v.arr.len; i++) {
    ...
    if (!parse_node_ref_id(p, args->v.arr.items[i], &aid)) {
      free(lowered);
      zasm_err_node_codef(p, call_id, n->tag, "sircc.zasm.call.bad_args", "sircc: zasm: %s node %lld arg[%zu] must be node ref", n->tag,
                          (long long)call_id, i);
      return false;
    }
    ZasmOp op = {0};
    ...
    if (!zasm_op_is_value(&op)) {
      if (op.k != ZOP_SLOT) {
        free(lowered);
        zasm_err_node_codef(p, call_id, n->tag, "sircc.zasm.call.arg.unsupported",
                            "sircc: zasm: %s node %lld arg[%zu] unsupported", n->tag, (long long)call_id, i);
        return false;
      }
      const char* reg = call_arg_reg(i);
      if (!reg) {
        free(lowered);
        zasm_err_node_codef(p, call_id, n->tag, "sircc.zasm.call.arg.too_many",
                            "sircc: zasm: %s node %lld has too many args for current ABI model", n->tag, (long long)call_id);
        return false;
      }
