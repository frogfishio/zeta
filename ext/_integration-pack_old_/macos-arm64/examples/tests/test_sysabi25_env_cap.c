...
int main(void) {
  ...
  if (!zi_proc_env25_register()) {
    fprintf(stderr, "zi_proc_env25_register failed\n");
    return 1;
  }

  const char *ep[] = {"A=1", "B=two", NULL};
  zi_runtime25_set_env(2, ep);

  uint8_t req[40];
  build_open_req(req, ZI_CAP_KIND_PROC, ZI_CAP_NAME_ENV);
  ...
  if (off < 8) {
    fprintf(stderr, "short env blob\n");
    return 1;
  }

  uint32_t version = read_u32le(buf + 0);
  uint32_t envc = read_u32le(buf + 4);
  if (version != 1 || envc != 2) {
    fprintf(stderr, "bad header: version=%u envc=%u\n", version, envc);
    return 1;
  }

  const char *expect[] = {"A=1", "B=two"};
  uint32_t p = 8;
  for (uint32_t i = 0; i < envc; i++) {
    ...
    if (sl != (uint32_t)strlen(expect[i]) || memcmp(buf + p, expect[i], sl) != 0) {
      fprintf(stderr, "env mismatch at %u\n", i);
      return 1;
    }
    p += sl;
  }
